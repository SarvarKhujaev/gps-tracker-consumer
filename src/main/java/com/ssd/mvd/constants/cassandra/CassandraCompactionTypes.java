package com.ssd.mvd.constants.cassandra;

import com.ssd.mvd.annotations.LinksToDocs;

@SuppressWarnings( value = "saves different types of compaction for Cassandra tables" )
@com.ssd.mvd.annotations.services.ImmutableEntityAnnotation
public final class CassandraCompactionTypes {
    @LinksToDocs(
            links = "https://docs.datastax.com/en/cql-oss/3.3/cql/cql_reference/cqlCreateTable.html#compactSubprop__compactionSubpropertiesLCS"
    )
    public static final String LEVELED_COMPACTION = "LeveledCompactionStrategy";

    @LinksToDocs(
            links = "https://docs.datastax.com/en/cql-oss/3.3/cql/cql_reference/cqlCreateTable.html#compactSubprop__compactionSubproperties"
    )
    public static final String DATE_TIERED_COMPACTION = "DateTieredCompactionStrategy";

    @LinksToDocs(
            links = "https://docs.datastax.com/en/cql-oss/3.3/cql/cql_reference/cqlCreateTable.html#compactSubprop__compactionSubpropertiesSTCS"
    )
    public static final String SIZE_TIERED_COMPACTION = "SizeTieredCompactionStrategy";

    @LinksToDocs(
            links = "https://docs.datastax.com/en/cql-oss/3.3/cql/cql_reference/cqlCreateTable.html#compactSubprop__compactionSubpropertiesTWCS"
    )
    public static final String TIME_WINDOW_COMPACTION = "TimeWindowCompactionStrategy";


    @SuppressWarnings(
            value = """
                    Use COMPACT STORAGE to store data in the legacy (Thrift) storage engine format to conserve disk space.
                        Use compact storage for the category table.
                                        
                        Example:
                            CREATE TABLE cycling.cyclist_category (
                               category text,
                               points int,
                               id UUID,
                               lastname text,
                               PRIMARY KEY (category, points))
                            WITH CLUSTERING ORDER BY (points DESC)
                               AND COMPACT STORAGE;
                    """
    )
    @LinksToDocs(
            links = "https://docs.datastax.com/en/cql-oss/3.3/cql/cql_reference/cqlCreateTable.html#compactSubprop__compactionSubpropertiesSTCS"
    )
    public static final String COMPACT_STORAGE = "COMPACT STORAGE";
}
